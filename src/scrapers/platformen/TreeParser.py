...
"""
import re, datetime as dt, os # core modules
import logging
...
class TreeParser():
    ...
    def __init__(self,url=None,tree=None,schemas=None,raise_error=True):
        ...
        datestr=dt.datetime.strftime(dt.datetime.now().date(),'%Y%m%d')
        ...
        # GET website html        
        if url==None and tree==None and schemas==None:
    ...
    def __serialize__(self,html_element):
       ...
       #body  
       if type(html_element) in [lxml.etree._ElementUnicodeResult]:
           v=str(html_element)
    ...
    def __transform_html_element_to_value__(self,html_element,element_transform_function=None):
       ...
       # transform element
       if element_transform_function!=None:
        ...
        try:
            ...
            if len(xpath)>0:
                ...
                if all_elements==True: # use the first result element
                    ...
                    value_input=element
                value=self.__serialize__(self.__transform_html_element_to_value__(value_input,transform)) # use all elements
